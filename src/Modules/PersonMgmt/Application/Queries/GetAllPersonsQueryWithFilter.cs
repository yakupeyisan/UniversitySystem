...
public class GetAllPersonsQueryWithFilter : IRequest<Result<PagedList<PersonResponse>>>
{
    public PagedRequest PagedRequest { get; set; }

    public string? FilterString { get; set; }

    public GetAllPersonsQueryWithFilter(
    ...
    public class Handler : IRequestHandler<GetAllPersonsQueryWithFilter, Result<PagedList<PersonResponse>>>
    {
        ...
        public async Task<Result<PagedList<PersonResponse>>> Handle(
            GetAllPersonsQueryWithFilter request,
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                _logger.LogInformation(
                    "Fetching persons with filter - Filter: {FilterString}, Page: {PageNumber}, Size: {PageSize}",
            ...
            {
                _logger.LogError(ex, "Error fetching persons with filter: {FilterString}", request.FilterString);
                return Result<PagedList<PersonResponse>>.Failure($"Error: {ex.Message}");
            }
        }
    }
}